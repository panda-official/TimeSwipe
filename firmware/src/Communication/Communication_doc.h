 ...
 * The current stream object type determines a storage format (text, binary, e.t.c)
 * 
 ...
 * Thus a communication "access point" can be created from already existing class methods with apropriate signature by binding them with
 * CCmdSGHandler for example
 *
 *
 * Since normally SPI bus is used for communication a kind of flow-control is also required. It is realised via CSyncSerComFSM
 */
