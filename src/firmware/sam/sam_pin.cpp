...
std::shared_ptr<Sam_pin> Sam_pin::FactoryPin(const Sam_pin::group nGroup,
...
void Sam_pin::SetPin(const Sam_pin::group nGroup, const Sam_pin::pin nPin,
  const bool bHow)
{
    if (bHow) {
...
bool Sam_pin::RbSetPin(const Sam_pin::group nGroup, const Sam_pin::pin nPin)
{
    using Uint = decltype(PORT->Group[0].OUT.reg);
    return PORT->Group[nGroup].OUT.reg & (Uint{1}<<nPin);
}

bool Sam_pin::GetPin(const Sam_pin::group nGroup, const Sam_pin::pin nPin)
{
    using Uint = decltype(PORT->Group[0].IN.reg);
    return PORT->Group[nGroup].IN.reg & (Uint{1}<<nPin);
}

void Sam_pin::ReleasePin(const Sam_pin::group nGroup, const Sam_pin::pin nPin)
{
