...
int _bcm_spi_rec_char(void)
{
   ...
   /* res=-1;
    if( bcm2835_peri_read(paddr) & BCM2835_SPI0_CS_RXD )
    {
        res=bcm2835_correct_order(bcm2835_peri_read_nb(fifo)); //???
    }
    if(res>0)
    {
        printf("-> %c\n", res);
...
int _bsm_spi_is_done(void)
{
    ...
    return (bcm2835_peri_read_nb(paddr) & BCM2835_SPI0_CS_DONE);
}
void _bsm_spi_cs(int how)
{
    volatile uint32_t* paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;

    bcm2835_peri_set_bits(paddr, (how ? BCM2835_SPI0_CS_TA:0), BCM2835_SPI0_CS_TA);
}
...
//-------------------------------------------VNK stuff: 30.05.2019----------------------------------------------
void _bcm_aux_spi_transfernb(const char *tbuf, char *rbuf, uint32_t len, int bCS) {
    volatile uint32_t* cntl0 = bcm2835_spi1 + BCM2835_AUX_SPI_CNTL0/4;
    volatile uint32_t* cntl1 = bcm2835_spi1 + BCM2835_AUX_SPI_CNTL1/4;
    volatile uint32_t* stat = bcm2835_spi1 + BCM2835_AUX_SPI_STAT/4;
    volatile uint32_t* txhold = bcm2835_spi1 + BCM2835_AUX_SPI_TXHOLD/4;
    ...
    while ((tx_len > 0) || (rx_len > 0)) {
        ...
        while (!(bcm2835_peri_read(stat) & BCM2835_AUX_SPI_STAT_TX_FULL) && (tx_len > 0)) {
            ...
            if (bCS) {
                bcm2835_peri_write(txhold, data);   //hold CS
            } else {
                bcm2835_peri_write(io, data);       //de-assert CS
            }
