...
{
	std::cerr << "Usage: 'sudo " << name << " [--config <configname>] [--binary] [--input <input_type>] [--output <outname>]'" << std::endl;
	std::cerr << "default for <configname> is ../config.json" << std::endl;
	std::cerr << "if --binary given then the resulting output will be written to a binary file" << std::endl;
